...
#[tauri::command]
pub async fn login_with_provider(app: tauri::AppHandle, provider: String) -> Result<UserInfo, String> {
...
fn load_oauth_configs(app: &tauri::AppHandle) -> Result<OAuthConfigs, String> {
    // Try multiple locations: resource directory (production), then current directory (development)
    let mut possible_paths = Vec::new();

    // First, try resource directory (for bundled production builds)
    if let Ok(resource_dir) = app.path().resource_dir() {
        possible_paths.push(resource_dir.join("oauth_config.json"));
    }

    // Second, try current directory (for development)
    if let Ok(current_dir) = std::env::current_dir() {
        possible_paths.push(current_dir.join("oauth_config.json"));
        // Also try src-tauri directory
        possible_paths.push(current_dir.join("src-tauri").join("oauth_config.json"));
    }

    // Third, try relative to executable
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            possible_paths.push(exe_dir.join("oauth_config.json"));
            possible_paths.push(exe_dir.join("resources").join("oauth_config.json"));
        }
    }
